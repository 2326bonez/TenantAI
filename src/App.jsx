...
function TenantApp({ signedIn = false, user, authConfigured }) {
  ...
  return <div className="app-shell">
    ...
    <main className="main-content">
      ...
      {!authConfigured && <div className="notice">Clerk is not configured in this environment. Add real values for <code>VITE_CLERK_PUBLISHABLE_KEY</code>, <code>CLERK_PUBLISHABLE_KEY</code>, and <code>CLERK_SECRET_KEY</code>, then restart the app to enable real accounts.</div>}
  ...
  return (
    <div className="workspace-grid">
      <section className="panel">
        ...
        <p className="muted">Share the facts and TenantAI will organize potential concerns, next steps, evidence to gather, and an editable message. This is not legal advice.</p>
        <form onSubmit={submit} className="analysis-form">
          <div className="field-row">
            <label>State<input required name="state" value={form.state} onChange={update} placeholder="e.g. California" /></label>
            <label>City <span className="optional">(optional)</span><input name="city" value={form.city} onChange={update} placeholder="Your city" /></label>
          </div>
          <label>What happened?<textarea required name="input" value={form.input} onChange={update} rows="8" placeholder="Include dates, communications, notices, and what you need help deciding..." /></label>
          <button className="primary" disabled={loading}>{loading ? "Analyzing…" : "Analyze my situation →"}</button>
        </form>
      </section>
      ...
      {result && (
        <section className="panel">
          <div className="panel-heading">
            <div>
              <p className="eyebrow">TenantAI analysis</p>
              <h2>What to do next</h2>
            </div>
          </div>
          {result.error && <div className="error-box"><strong>Analysis needs attention</strong><p>{result.error}</p>{result.note && <small>{result.note}</small>}</div>}
          {!result.error && (
            <>
              <div className="ai-disclaimer">AI-generated guidance · Review local requirements and consider qualified legal help for decisions.</div>
              {issues.length > 0 && <ResultSection title="Potential housing concerns">{issues.map((item, index) => <div className="result-card" key={index}><strong>{item.title}</strong><span className="risk">{item.riskLevel || "Review"}</span><p>{item.description}</p></div>)}</ResultSection>}
              <ResultSection title="What to do next"><ol>{steps.map((step, index) => <li key={index}>{step}</li>)}</ol></ResultSection>
              {result.communication?.template && <ResultSection title="Suggested communication"><textarea ref={messageRef} className="template" defaultValue={result.communication.template} /><button type="button" className="secondary" onClick={copyMessage}>{copied ? "Copied" : "Copy message"}</button></ResultSection>}
              {resources.length > 0 && <ResultSection title="Helpful resources"><ul className="resource-list">{resources.map((item, index) => <li key={index}><a href={item.url} target="_blank" rel="noreferrer">{item.name} ↗</a><p>{item.description}</p></li>)}</ul></ResultSection>}
            </>
          )}
        </section>
      )}
