...
function buildFallbackReport({ state, city, input }) {
  ...
  return {
    ...
    issues: [
      {
        title: "Housing concern requiring review",
        description: `Based on the information provided for ${normalizedCity}, ${normalizedState}, your situation may require documentation, follow-up, and a careful review of your tenant rights.`,
        riskLevel: "Medium",
      },
    ],
    steps: [
      "Document all communications, dates, and any evidence related to the issue.",
      "Collect photos, videos, receipts, and messages that support your situation.",
      "Draft a clear written request or notice to your landlord or property manager.",
      "Research the tenant protections that apply in your state and city.",
    ],
    communication: {
      template: `Dear [Landlord Name],\n\nI am writing to raise a concern regarding my rental unit in ${normalizedCity}, ${normalizedState}.\n\n${input}\n\nI request that this matter be addressed promptly and in writing. Please let me know how you plan to resolve it.\n\nSincerely,\n[Your Name]`,
      tone: "professional",
    },
    resources: [
      {
        name: "Tenant Union",
        url: "https://www.tenantunion.org/",
        description: "Resources and guidance for renters dealing with housing disputes.",
      },
      {
        name: "LawHelp",
        url: "https://www.lawhelp.org/",
        description: "Find local legal aid and tenant-rights assistance in your area.",
      },
    ],
  };
}
...
function normalizeReport(report, fallback) {
  ...
  const issues = Array.isArray(report?.issues) && report.issues.length
    ? report.issues.map((issue) => ({
        ...
        description: issue?.description || "No issue details were provided.",
  ...
  const resources = Array.isArray(report?.resources) && report.resources.length
    ? report.resources.map((resource) => ({
        ...
        url: resource?.url || "https://www.lawhelp.org/",
        description: resource?.description || "Additional support information.",
      }))
    : fallback.resources;
...
app.post("/api/analyze", async (req, res) => {
  ...
  if (!openai) {
    return res.status(503).json({
      ...
      note: "Add OPENAI_API_KEY to your environment to enable full analysis.",
    });
  }
  ...
  try {
    ...
    const report = parsedResponse && typeof parsedResponse === "object"
